...
/**
 * GUI to make enrollment from database
 *
 */
public class EnrollmentController {
	// primary stage used in Enrollment
    private Stage stage;
    // list of all Enrollment made in database, used for GUI table
    private ObservableList<Enrollment> enrollments = FXCollections.observableArrayList(EnrollmentDAO.fetchAllEnrollments());
    // get the database for Enrollment
    private EnrollmentDAO enrollmentDAO = new EnrollmentDAO();
    
    /**
     * constructor for Enrollment GUI
    ...
    /**
     * show the window for enrollment 
     */
    public void show() {
        ...
        enrollBtn.setOnAction(e -> {
            try {
                ...
                showAlert("Invalid ID Format. Use integers and don't leave any text field empty.");
            }
        });
        ...
        deleteBtn.setOnAction(e -> {
            try {
                ...
                // check to see if enrollmentID exists
                Enrollment existing = enrollments.stream()
                        .filter(c -> c.getEnrollmentID() == id)
                        .findFirst()
                        .orElse(null);
                    if (existing == null) {
                    	showAlert("Enrollment with ID " + id + " not found.");
                        return;
                    }
            	...
            	showAlert("Invalid ID Format. Don't use letters");
            }
        });
        ...
        searchBtn.setOnAction(e -> {
            ...
            if (!"Select".equals(cidOp) && !cidVal.isEmpty()) {
                conditions.add("studentID " + convertOperator(cidOp) + " " + cidVal);
            }

            String sidOp = studentIDOperatorCombo1.getValue();
            String sidVal = studentIDChoose1.getText().trim();
            if (!"Select".equals(sidOp) && !sidVal.isEmpty()) {
                conditions.add("courseID " + convertOperator(sidOp) + " " + sidVal);
            }
